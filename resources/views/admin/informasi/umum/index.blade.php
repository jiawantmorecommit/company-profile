        ...
        <div class="bg-white overflow-hidden shadow-sm sm:rounded-lg">
            <div class="p-6 text-gray-900">
                <div class="flex justify-between items-center mb-6">
                    ...
                    <a href="{{ route('admin.informasi.umum.create') }}" class="px-4 py-2 bg-blue-600 text-white rounded-md hover:bg-blue-700 transition duration-150 ease-in-out">
                        Tambah Baru
                    </a>
                </div>
                                ...
                                <tr>
                                    ...
                                    <td class="px-6 py-4 whitespace-no-wrap border-b border-gray-200">
                                        @if($information->images)
                                            <img src="{{ asset('storage/' . $information->images) }}" alt="{{ $information->title }}" class="h-10 w-auto object-cover">
                                        @else
                                    ...
                                    <td class="px-6 py-4 whitespace-no-wrap border-b border-gray-200">
                                        @if($information->pdf)
                                            <a href="{{ route('informasi.download', ['type' => 'umum', 'id' => $information->id]) }}" class="text-blue-600 hover:underline" target="_blank">
                                                Lihat PDF
                                            </a>
                                        @else
                                    ...
                                    <td class="px-6 py-4 whitespace-no-wrap border-b border-gray-200 text-sm">
                                        <div class="flex space-x-2">
                                            <a href="{{ route('admin.informasi.umum.edit', $information->id) }}" class="text-indigo-600 hover:text-indigo-900">Edit</a>
                                            <form action="{{ route('admin.informasi.umum.destroy', $information->id) }}" method="POST" onsubmit="return confirm('Apakah Anda yakin ingin menghapus informasi ini?');">
                                                @csrf
                                                @method('DELETE')
